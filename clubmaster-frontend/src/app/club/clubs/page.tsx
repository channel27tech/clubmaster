    ...
    <div className="min-h-screen bg-[#333939] flex flex-col w-full max-w-[400px] mx-auto relative">
      ...
      <div className="fixed top-0 left-1/2 -translate-x-1/2 w-full max-w-[400px] z-10">
        ...
        <div className="bg-[#333939] p-4 flex items-center">
          ...
          <h1 className="text-[#FAF3DD] text-xl font-medium flex-1 text-center mr-6">Clubs view</h1>
        </div>

        {/* Create Club Button - reduced vertical padding from py-3 to py-2 */}
        <div className="px-4 py-1 bg-[#333939]">
          <button 
            onClick={() => router.push('/club/create')}
            className="w-full py-4 bg-[#4A7C59] text-[#FAF3DD] text-base font-medium border-2 border-[#E9CB6B]"
        ...
        {/* Search Bar - reduced bottom padding from pb-3 to pb-2 */}
        <div className="px-4 pb-2 pt-1 flex gap-2 bg-[#333939]">
          <div 
            className="flex items-center bg-[#4C5454] px-3 py-2 flex-1"
            style={{ borderRadius: '0.75rem' }}
          >
            <input
              className="flex-1 bg-transparent outline-none text-[#D9D9D9] placeholder-[#D9D9D9] text-sm"
              placeholder="Search..."
            />
          ...
          <button 
            className={`bg-[#4C5454] p-2 ${showFilters ? 'bg-[#4A7C59]' : 'bg-[#4C5454]'}`}
            style={{ borderRadius: '0.75rem' }}
        ...
        {showFilters && (
          <div className="bg-[#1F2323] absolute right-4 mt-1 w-56 rounded-lg shadow-lg z-20 border border-[#505454] overflow-hidden">
            <div className="p-1">
              <button 
      ...
      <div className="flex-1 px-4 pb-16 pt-44 overflow-y-auto">
        {clubData.map((club, index) => (
          <div 
            key={club.id} 
            className="bg-[#4C5454] overflow-hidden p-3 flex items-center mb-3 mt-1" 
            style={{ borderRadius: '0.75rem' }}
            onClick={() => router.push('/club/players')}
          >
            <div className="w-16 h-16 rounded-full overflow-hidden bg-white flex-shrink-0 mr-4">
            ...
            <div className="flex-1">
              <div className="flex items-center">
                <h3 className="text-[#FAF3DD] font-medium">{club.name}</h3>
                {club.locked && (
              ...
              <div className="flex text-xs items-center mt-1">
                ...
                <span className="text-[#D9D9D9]">{club.members} members</span>
                ...
                <span className="text-[#D9D9D9]">{club.location}</span>
              </div>
              
              <div className="flex mt-2 text-xs items-center">
                {/* Achievement medal icon and rank */}
                <span className="mr-2 text-lg">🥇</span>
                <span className="text-[#D9D9D9] font-medium">{club.rank}</span>
                
                {/* Pointing finger instead of check mark */}
                <span className="mx-3 text-amber-400">👉</span>
                
                {/* Points with updated color */}
                <span className="text-[#D9D9D9]">{club.points}</span>
              </div>
            </div>
          </div>
        ))}
